...
The sensor is node 10 at 125 kbps. Two ways to get measurements out of it:
  ...
  stream    decode TPDO1 at 10 ms. Needs the sensor in Operational, so this
            sends NMT Start addressed to node 10 only, and puts it back to
            Pre-operational on exit.
...
def live_values(bus, node, combi):
    ...
    words = [rd(bus, node, 0x2021, s) for s in (1, 2, 3)]
    if any(w is None for w in words):
        return None
    nlcp = rd(bus, node, 0x2021, 4)
    status = rd(bus, node, 0x2022, 0)
    if nlcp is None or status is None:
        return None
    data = struct.pack("<HHHBB", *words, nlcp & 0xFF, status & 0xFF)
...
def snapshot(bus, node):
    ...
    cob = rd(bus, node, 0x1800, 1)
    ttype = rd(bus, node, 0x1800, 2)
    timer = rd(bus, node, 0x1800, 5)
    if cob is not None:
        state = f"{BAD} DISABLED" if cob & 0x80000000 else f"{OK} enabled"
        print(f"    1800h TPDO1              COB-ID 0x{cob & 0x7FF:03X} {state}, "
              f"type 0x{ttype:02X}, event timer {timer} ms")
    ...
    for sub, i in ((0x0B, 1), (0x0C, 2), (0x0D, 3)):
        v = rd(bus, node, 0x2021, sub, signed=True)
        if v is not None:
...
def stream(bus, node, seconds, start_nmt):
    ...
    if n == 0:
        ...
        print("      The sensor only transmits PDOs in Operational. If you passed")
        print("      --no-nmt, it was probably still in Pre-operational.")
...
def main():
    ...
    ap.add_argument("--no-nmt", action="store_true",
                    help="stream: do not send NMT Start (sensor already Operational)")
    args = ap.parse_args()
    ...
    try:
        ...
        return stream(bus, args.node, args.seconds, not args.no_nmt)
    except KeyboardInterrupt:
        print("\n    interrupted")
        return 1
    finally:
        if args.mode == "stream" and not args.no_nmt:
