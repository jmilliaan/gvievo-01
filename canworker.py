...
# The four helpers below live in debug_commands/. Despite the directory name
# they are RUNTIME dependencies of the web app, not just standalone debug
# scripts - moving or renaming that directory breaks the server.
sys.path.insert(0, os.path.join(_ROOT, "debug_commands"))
...
# values, and agv-profile.json to change them.
...
class TpdoTap:
    """Bus wrapper that siphons sensor TPDO1 frames off before the SDO helpers
    can discard them.
    ...
    sensor's 100 Hz stream still reaches the decoder. Without it, sdo_read's
    "drain anything stale" loop would silently eat every sensor frame.
    """

    def __init__(self, bus, on_pdo):
        self._bus = bus
        self._on_pdo = on_pdo
...
class Controller:
    ...
    def __init__(self):
        ...
        # per edge rather than one per 5 Hz poll.
    ...
    def _run(self):
        try:
            raw, how = open_bus(config.CAN_BITRATE)
        except Exception as e:
            with self._lock:
                self.error = str(e)
            events.error(f"CAN bus unavailable: {e}")
            return
        self.bus = TpdoTap(raw, self._on_pdo)
        with self._lock:
            self.how = how
            self.error = None
        ...
        try:
            while not self._stop_evt.is_set():
                ...
                stats = health.tick(t_iter, spent, self._sensor_seen, armed)
                if stats:
                    with self._lock:
    ...
    def _on_pdo(self, msg):
        ...
        with self._lock:
            ...
            self._sensor_seen += 1
            self._sensor_last = time.monotonic()
