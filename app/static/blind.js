...
function nearestTape(sensor) {
  const t = (sensor && sensor.tracks) || [];
  if (!t.length) return null;
  return t.reduce((a, b) => Math.abs(b.pos_mm) < Math.abs(a.pos_mm) ? b : a).pos_mm;
}

function showImu(m) {
  setText('imu-status', m.available ? 'IMU reading' : `IMU not available - ${m.reason || 'unknown'}`);
  const v3 = a => a ? a.map(x => fx(x, 2)).join(' / ') : '–';
  setText('imu-rp', m.available ? `${fx(m.roll_deg, 2)} / ${fx(m.pitch_deg, 2)}` : '–');
  setText('imu-yaw', m.available ? fx(m.yaw_deg, 2) : '–');
  setText('imu-acc', m.available ? v3(m.accel_mps2) : '–');
  setText('imu-gyro', m.available ? v3(m.gyro_dps) : '–');
  setText('imu-temp', m.available && m.temp_c !== null ? String(m.temp_c) : '–');
  setText('imu-age', m.age_s === null || m.age_s === undefined ? '°C' : `°C · read ${fx(m.age_s, 1)} s ago`);
}
