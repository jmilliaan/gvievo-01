...
def test_plan_maths():
    ...
    n = uturn.counts_for_angle(90, CPR)
    check("a 90 deg pivot matches the U-turn geometry, wheels opposed",
...
def test_imu_display():
    print("\nMLS IMU is decoded for display only")
    d = canworker._decode_imu([15708, 0xFFFF, 0], [2048, 0, 0xF800], [16, 0, 0], 0xE2)
    check("angles are 1/10000 rad per bit, signed",
          abs(d["roll_deg"] - 90.0) < 0.01 and d["pitch_deg"] < 0 and d["yaw_deg"] == 0)
    check("acceleration is 2^-11 g per bit, signed",
          abs(d["accel_mps2"][0] - 9.80665) < 1e-9 and abs(d["accel_mps2"][2] + 9.80665) < 1e-9)
    check("angular rate is 125*2^-11 deg/s per bit", abs(d["gyro_dps"][0] - 0.9765625) < 1e-12)
    check("temperature is a signed byte", d["temp_c"] == -30)

    c = canworker.Controller()
    writes = []
    c._write = lambda *a, **k: writes.append(a)
    table = {(0x2006, 2): 1, (0x2070, 1): 25}
    for idx in (0x2030, 0x2033, 0x2034):
        for sub in (1, 2, 3):
            table[idx, sub] = 0
    c._read = lambda node, index, sub=0, **kw: table.get((index, sub))

    def poll(at, streaming=True):
        c._sensor_last = at - 0.1 if streaming else at - 5.0
        c._poll_imu(at)

    poll(50.5)
    check("a V5 MLS with MEMS on reports its IMU",
          c._imu["available"] and c._imu["temp_c"] == 25, str(c._imu))
    table[0x2006, 2] = 0
    c._imu_mems = None
    poll(51.0)
    check("MEMS off is named, not faked", not c._imu["available"] and "MEMS off" in c._imu["reason"])
    del table[0x2006, 2]
    c._imu_mems = None
    poll(51.5)
    check("no 2006h:02 answer means no IMU on this firmware",
          not c._imu["available"] and "V5" in c._imu["reason"])
    poll(60.0, streaming=False)
    check("an MLS that is not streaming is not polled",
          not c._imu["available"] and "not streaming" in c._imu["reason"])
    check("the IMU poll never writes the MLS", writes == [])
    check("the snapshot carries it with an age",
          "mls_imu" in c.snapshot() and "age_s" in c.snapshot()["mls_imu"])
...
def test_blind_page_sets_but_cannot_start():
    ...
    check("/blind renders", 'id="blind-set"' in body and "MLS IMU" in body)
...
TESTS = [test_plan_maths, test_closed_loop_and_odometry, test_controller_panel_rules,
         test_imu_display, test_blind_page_sets_but_cannot_start]
