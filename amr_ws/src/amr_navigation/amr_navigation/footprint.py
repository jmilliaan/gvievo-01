...
def swept_line(
    ...
    return dilate(mask, int(round(fp.margin_m / grid.meta.resolution)))
...
def swept_rotation(grid: Grid, fp: Footprint, pivot: tuple[float, float]) -> np.ndarray:
    ...
    return dilate(mask, int(round(fp.margin_m / res)))
...
class Clearance:
    ...
    @property
    def clear(self) -> bool:
        return self.occupied == 0 and self.unknown == 0 and self.keepout == 0


def check(grid: Grid, mask: np.ndarray, keepout: Grid | None = None) -> Clearance:
    ...
    return Clearance(occ, unk, ko, int(mask.sum()))
