...
energises: the horn coil, raised while motion is commanded. It sits after the
reads for the reason a PLC puts it there - the outputs written in a scan are
the ones decided from the inputs read in that same scan, so a coil can never be
driven from an image nobody has refreshed.
